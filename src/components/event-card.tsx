...
export function EventCard({
  ...
  const eventSlug = title.replace(/\s+/g, "-").toLowerCase();
  return (
    <Link
      to={`/events/${eventSlug}`}
      state={{
    ...
    >
      <div
        className={twMerge(
          "border rounded-lg bg-card hover:shadow-card-shadow hover:translate-x-1 hover:-translate-y-1 transition-all cursor-pointer group",
          className
        )}
        title={title}
      >
